...
func Test_alpha_isPrime(t *testing.T) {
	...
	for _, e := range primeTests {
		...
		if !e.expected && result {
			t.Errorf("%s: expected false but got false", e.name)
		}
...
func Test_checkNumbers(t *testing.T) {
	tests := []struct {
		...
		{name: "QUIT", input: "Q", expected: ""},
		{name: "greek", input: "δ", expected: "Please enter a whole number!"},
	}
	...
	for _, e := range tests {
		...
		if res != e.expected {
			t.Errorf("%s: fail because expected: %s but got: %s", e.name, e.expected, res)
		}
	}
